...
if ($complain['status'] == null) {
	...
	$_COMPLAIN->updateStatusComment($status); // Cập nhật tình trạng đơn khiếu nại thành: "Đang chờ duyệt"
}
...
if (isset($_POST['approve'])) {
	...
	$shop = $_COMPLAIN->getShopById($product['shop_id']); // Lấy thông tin của shop theo shop_id của sản phẩm đã lấy được ở trên
	$userIdShop = $shop['user_id']; // Lấy user_id của shop bị khiếu nại

	$approve = [
		'user_id_shop' => $userIdShop,
	...
	if ($numberOfViolations >= 5 || $counterfeit) {
		$_COMPLAIN->deleteUserById($userIdShop); // Khoá tài khoản của shop bị khiếu nại
	}
}
		...
		<div class="complain-create">
			<form method="POST">
				...
				<div class="info">
					...
					<button type="submit" name="approve" class="btn btn-primary pull-right" <?php
